...
export default function ProvidersSettingsPage() {
  ...
  const [selectedProvider, setSelectedProvider] = useState<IAgentProvider | undefined>();
  ...
  const handleProviderDialogOpenChange = (open: boolean): void => {
    if (open) return;

    handleCloseProviderDialog();
  };

  const handleCloseOAuthContent = (cancelLogin: boolean): void => {
    handleCloseProviderDialog(cancelLogin);
  };

  const startOAuthLogin = async (provider: IAgentProvider) => {
    await startOAuthLoginMutation({providerId: provider.id});
    setProviderDialogView("oauth");
  };

  const handleDisconnect = async (provider: IAgentProvider) => {
    await logoutMutation.mutateAsync({providerId: provider.id});
  };

  const handleConnect = async (provider: IAgentProvider) => {
  ...
  const renderProviderDialogContent = () => {
    if (providerDialogView === "method") {
      return <ProviderConnectMethodContent onSelect={handleConnectMethod} isStartingOAuthLogin={isStartingOAuthLogin} />;
    }

    if (providerDialogView === "api_key") {
      return <ProviderApiKeyContent onClose={handleCloseProviderDialog} provider={selectedProvider} />;
    }

    if (providerDialogView === "oauth") {
      return <ProviderOAuthLoginContent loginSessionId={loginSessionId} onClose={handleCloseOAuthContent} />;
    }
