...
export default function FileTreeRow(props: FileTreeRowProps) {
  ...
  return (
    <Button
      ...
      role="treeitem"
      style={{paddingLeft: `${6 + depth * ROW_INDENT_PX}px`}}
      variant="primary"
    >
      ...
      <span className={cn("flex min-w-0 flex-1 text-left", node.entry?.status === "deleted" && "line-through")}>
      ...
      {node.entry && <GitStatusBadge status={node.entry.status} />}
    </Button>
  );
}
