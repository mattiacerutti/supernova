...
import {AgentRpcProtocolClientService, getAgentRpcClient} from "@/rpc/agent-rpc-client";
...
const sharedRpcRuntime = {
  runPromiseExit: async <TSuccess, TError>(effect: Effect.Effect<TSuccess, TError, AgentRpcProtocolClientService>, options?: {readonly signal?: AbortSignal | undefined}) => {
    const rpcClient = await getAgentRpcClient();
    return rpcClient.runExit((client) => Effect.provideService(effect, AgentRpcProtocolClientService, client), options);
  },
