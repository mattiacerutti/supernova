...
function makePiSessionsHarness(input?: {
  ...
  messages?: unknown[];
  prompt?: (message: string, context: {emit: (event: PiSessionEvent) => void; messages: unknown[]}) => Promise<void>;
    ...
    createAgentSession: vi.fn(async () => ({
      session: {
        ...
        prompt: async (message: string) => {
          if (input?.prompt) return input.prompt(message, {emit: (event) => subscriber?.(event), messages});
  ...
  return {abort, appendSessionInfo, dispose, getBranch, messages, piSdk, sendMessage, sessionsLive, setModel, setThinkingLevel, unsubscribe};
}
...
describe("sendSessionMessage", () => {
  ...
  it("streams only the active live turn while keeping base history in ready", async () => {
    ...
    const streamPromise = Effect.runPromise(
      Effect.gen(function* () {
        const sessions = yield* SessionsService;
        yield* Stream.runForEach(sessions.sendMessage({message: "Live request", model: {id: "claude-sonnet", providerId: "anthropic"}, sessionId: "session-1"}), (event) =>
          Effect.sync(() => {
            events.push(event);
